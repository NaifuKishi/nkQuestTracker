...
function privateVars.internal.craftinglist (internalFlag)
	...
	if internalFlag ~= true and uiElements.craftingList:GetVisible() == true then 
		uiElements.craftingList:SetVisible(false)
		return
	end

	local category = uiElements.questLog:GetCategory('crafting')
  ...
  privateVars.missingItemList = {}
	...
	local grid = uiElements.craftingList:GetGrid() 
	
	grid:SetRowPos(1, false)
	grid:SetCellValues(gridValues)
	uiElements.craftingList:SetVisible(true)
...
function privateVars.internal.craftingUI ()
	...
	local ui = UI.CreateFrame("Frame", name, uiElements.questLog)
	ui:SetWidth(300)
	ui:SetHeight(uiElements.questLog:GetHeight())
